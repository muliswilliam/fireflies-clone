...
This bounds worst-case daily spend to a few dollars with one SQL count and no extra infrastructure. The count and insert run in one transaction under a Postgres advisory lock, so two requests arriving together at the limit cannot both slip through. It does not stop one person from consuming the whole allowance, which is acceptable for a demo whose reviewers are a handful of people.
